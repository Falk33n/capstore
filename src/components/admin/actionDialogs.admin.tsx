...
const AdminUserEditPropsMapped: Record<keyof AdminEditProps, string> = {
...
export function AdminEditAction({ user }: { user: AdminEditProps }) {
  return (
    <Dialog>
          ...
          {Object.entries(user).map(([prop, val], i) => (
            <div key={i} className='grid grid-cols-4 items-center gap-4'>
              <Label htmlFor={prop} className='text-right'>
                {AdminUserEditPropsMapped[prop as keyof AdminEditProps]}
              </Label>
              <Input
                id={prop}
